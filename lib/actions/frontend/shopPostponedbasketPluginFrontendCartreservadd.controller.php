...
class shopPostponedbasketPluginFrontendCartreservaddController extends waController
{
    /**
     * @var shopCart
     */
    ...
    {
        $code = waRequest::cookie('shop_cart_reserv');
        if (!$code) {
            $code = md5(uniqid(time(), true));
            // header for IE
            wa()->getResponse()->addHeader('P3P', 'CP="NOI ADM DEV COM NAV OUR STP"');
            // set cart cookie
            wa()->getResponse()->setCookie('shop_cart_reserv', $code, time() + 30 * 86400, null, '', false, true);
        }
